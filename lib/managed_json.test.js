...
describe('Non-assignable types', () => {
  ...
  test('Creating a managed object that contains a function must throw', () => {
    expect(() => ManagedJson.create({ a: 1, fn: function(){return 1;} })).toThrow();
  });

  test('Creating a managed object that contains a function in a nested object must throw', () => {
    expect(() => ManagedJson.create({ a: 1, b: { fn: function(){return 1;} }})).toThrow();
  });
  ...
  test('Creating a managed object that contains a symbol property in a nested object must throw', () => {
    const sym = Symbol();
    expect(() => ManagedJson.create({ a: { [sym]: 1 }})).toThrow();
  });

  test('Creating a managed object that contains a symbol property in a nested array must throw', () => {
    const sym = Symbol();
